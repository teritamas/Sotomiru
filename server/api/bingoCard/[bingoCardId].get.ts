...
import { CompleteBingoLine, ReachLine } from "@/server/models/bingo/response";
...
export default defineEventHandler(async (event) => {
  try {
    ...
    // ビンゴかリーチが発生しているかを確認する
    const reachOrBingoLines = checkBingoOrReachLines(bingoCard!);

    return {
      bingoCard: bingoCard,
      completeBingoLines: reachOrBingoLines.completeBingoLines,
      reachLines: reachOrBingoLines.reachLines,
    };
  } catch (e) {
    console.error(e);
  }
 ...
 */
function checkBingoOrReachLines(bingoCard: BingoCard) {
